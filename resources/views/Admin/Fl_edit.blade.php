...
<main class="flex-1 p-6 overflow-y-auto">
    ...
    <form action="{{ url('/admin/flash-sale/' . $sale->id) }}" method="POST" class="bg-white p-6 rounded shadow w-full max-w-xl">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" value="{{ $sale->name }}" required class="w-full border px-3 py-2 rounded" />
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium mb-1">Thời gian bắt đầu</label>
            <input type="datetime-local" name="start_time" value="{{ \Carbon\Carbon::parse($sale->start_time)->format('Y-m-d\TH:i') }}" required class="w-full border px-3 py-2 rounded" />
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium mb-1">Thời gian kết thúc</label>
            <input type="datetime-local" name="end_time" value="{{ \Carbon\Carbon::parse($sale->end_time)->format('Y-m-d\TH:i') }}" required class="w-full border px-3 py-2 rounded" />
        </div>
